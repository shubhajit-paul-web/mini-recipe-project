	...
	const handleToggleFavorite = ({target}) => {
		...
		setRecipe((prevRecipes) => {
			return prevRecipes.map((recipe) => {
				if (recipe.id === cardID) recipe.isFavorite = !recipe.isFavorite;
				return recipe;
			});
		});
	};
	...
	return (
		<div className="relative w-[21rem] h-fit rounded-2xl p-4 bg-white shadow-2xl shadow-zinc-300 transition-transform duration-200 hover:scale-[102%]">
			...
			<div className="py-3 pb-7">
				<div className="text-2xl font-bold">{recipeName || "[Recipe Name]"}</div>
				...
				<div className="mt-1.5 text-xs flex gap-1.5 font-medium overflow-x-auto scroll-0" style={{scrollbarWidth: "none"}}>
					{tagsArr &&
